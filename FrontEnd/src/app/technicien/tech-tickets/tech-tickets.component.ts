...
public ticketsList : Ticket[] = tickets;
  ...
  displayedColumns: string[] = ['spécialité','description', 'Priorité','Date de création','consulter'];
...
//   }})
  ...
  onefiltrerSubmit(){
    this.userService.getTicketsfiltrertech(this.formflitrer.value.groupe_id,JSON.parse(this.technicienId).userId).subscribe(
      (resultatTicket) => {
        this.fetchedTicket = resultatTicket;
       // this.dtTrigger.next();
       this.dataSource = new MatTableDataSource(this.fetchedTicket);
...
//getTicketsfiltrer(id:String)
    ...
    commencertraitement(ticket:string){
      this.userService.changerertat(ticket);
    }



  onCloturerTiket(userId:string,demandeur:string){

    this.userService.EditCloturer(this.formclourer.value.note,userId);
    this.userService.getUser(demandeur).subscribe(
      (resultat:any) => {
        console.log(resultat);
        this.techmail = resultat;
        this.userService.envoyermailuser(resultat);
      });
    this.showTicketcloturation = false;
    this.showTicketDetails = false
