...
class _DiscussionInfoDetailState extends State<Oneticket> {
  ...
  sendemail() async {
   ...
   var headers = {
    ...
    var uri = Uri.parse("http://localhost:3000/api/sendmailticket");
...
getTechnicientoaffect() async {
    ...
    var url = "http://localhost:3000/api/users/"+selectedValue3+"/getuser";
    var uri = Uri.parse(url);
    // var request = http.get(uri, headers: headers);
    var request = http.get(uri, headers: headers);
    var response = await request.timeout(Duration(seconds: 10));
    setState(() {
      tech = jsonDecode(response.body);
    });
  }

// http://localhost:3000/api/users/${id}/getuser
// getuser()
...
Encourstraitement() async {
    ...
    var uri = Uri.parse("http://localhost:3000/api/tickets/" +
        widget.info["_id"] +
        "/changeretatencoursdetraitement");
   // var request = http.delete(uri, headers: headers);

       var request = http.put(uri, headers: headers);
    // Navigator.push(
    //     context, MaterialPageRoute(builder: (context) =>AllusersAdmin() ));
  }
...
Future<User> fetchUser2() async {
 ...
 getDate3() {
  ...
  DateTime dateTime = getFormattedDateFromFormattedString(
      value: widget.info['Dateaffectation'],
      currentFormat: "yyyy-MM-ddTHH:mm:ssZ",
  //    desiredFormat: "yyyy-MM-dd HH:mm:ss");
   desiredFormat: "yyyy-MM-dd HH:mm");
   if(widget.info['Dateaffectation']!=null){
  return dateTime;}
  else {
    // If the server did not return a 200 OK response,
    // then throw an exception.
 ...
 @override
  ...
  backgroundColor: HexColor('#f6f6f6'),
          ...
          // ),
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                return Text('Demandeur : '+snapshot.data.name+" "+ snapshot.data.prenom );
           ...
           FutureBuilder<User>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                return Text('Technicien :    ' +snapshot.data.name +" "+snapshot.data.prenom);
              } else if (snapshot.hasError) {
                return Text('${snapshot.error}');
              }
          ...
          SizedBox(height: 15), 
               if ((widget.info['Datecreaation']!=widget.info['Dateaffectation'])&&(widget.info['Dateaffectation']!=null))
                 
                       Text(
              //  widget.info['Datecreaation'],
              'réaffecter le : '+getDate3().toString(),
                style: TextStyle(
                  color: textColor,
                    // backgroundColor: Colors.white,
                  fontSize: 18,
  ...
  children:<Widget>[
    new Container(
     ...
     Encourstraitement();
        ...
        label: const Text('en cours de traitement'),
        // icon: const Icon(Icons.plus_one_rounded),
 ...
 //ClotureTicket();
  ...
  children: <Widget>[
    new Container(
      ...
      child: FloatingActionButton.extended(
        onPressed: () async{
    await Affecterticket();
await getTechnicientoaffect();
log(tech["name"]);
await  sendemail();
 await  Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) =>AllticketsAdmin() ));
